    ...
    <div className="w-full max-w-7xl mx-auto px-4 md:px-8 py-6 space-y-6">
      ...
      <div className="bg-white rounded border border-slate-200 p-6 shadow-sm space-y-4">
        <h3 className="text-base font-bold text-[#0B192C] font-serif border-b border-slate-200 pb-3 flex items-center justify-between">
          ...
          <span className="text-xs font-bold text-[#1E3A8A]">{FIXTURE_DATA_SOURCES.length} Registered Sources</span>
        </h3>
            ...
            <tbody className="divide-y divide-slate-100 text-slate-700">
              {FIXTURE_DATA_SOURCES.map((ds, idx) => (
                <tr key={idx} className="hover:bg-slate-50">
                  <td className="py-3 px-4 font-bold text-[#0B192C]">{ds.dataset_name}</td>
                  ...
                  <td className="py-3 px-4">
                    <span className={`px-2 py-0.5 rounded text-[10px] font-bold ${
                      ds.license.includes('Real') || ds.license.includes('Open') ? 'bg-blue-100 text-[#1E3A8A]' : 'bg-amber-100 text-amber-900 border border-amber-300'
                    }`}>
                      {ds.license}
                    </span>
                  </td>
